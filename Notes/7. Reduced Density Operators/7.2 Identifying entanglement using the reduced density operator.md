...
It is easy to check that a state is not entangled - it is a [[3.1 The Hilbert space of two-party quantum systems - the tensor product#^739f8e|product state]] - then the reduced density operator is a *pure state* - therefore *not* a mixed state. For example, a state ^039366
